...
module.exports = function(ViewBuyerUser) {
	/** 
	* 관리자회원을 조회하는 함수;
	* @function
	* @param {filter} Object
	<code>
	{
		where : {
			and : [
				{ email : /12k4@naver.com/ },//회원아이디(like 검색)
				{ companyName : /Umm/ },//업체명(like 검색)
				{ userType : "SA" },//회원구분
				{ ctId : "KR" },//회원회사의국가
				{ status : "Y" },//회원상태
	...
	*/
	ViewBuyerUser.getList = function( filter ){
		var o = filter | {};
		ViewBuyerUser.find(o,function(err,data){
			if(err) throw(err);
			console.log( data );
			return data;
		})
	...
	*/
	ViewBuyerUser.getById = function(id){
		ViewBuyerUser.findOne({ where : { id : id }},function(err,data){
			if(err) throw(err);
			console.log( data );
			return data;
		})
